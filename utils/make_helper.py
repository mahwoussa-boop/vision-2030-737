...
def export_to_make_format(df, section_type: str = "update") -> List[Dict]:
    ...
    for _, row in df.iterrows():
        ...
        # ── رقم المنتج ────────────────────────────────────────────────────
        # C-02 Fix: دعم "No." و "no." كأعمدة معرفات لمتجرك
        ...
        if section_type == "raise":
            ...
            # L-02 Fix: في حال كان سعرنا أقل، نرفع السعر لنقترب من المنافس لكن مع الحفاظ على ميزة تنافسية (مثلاً 5 ر.س أقل)
            # إذا كان الفارق كبيراً، نرفع السعر لزيادة الهامش مع البقاء أرخص
            COMPETITIVE_MARGIN = 5.0
            _target = round(comp_price - COMPETITIVE_MARGIN, 2) if comp_price > COMPETITIVE_MARGIN else our_price
            price = _target if (our_price > 0 and _target > our_price) else our_price
